...
import com.example.libraryservicemanager.function.TriConsumer;
import com.example.libraryservicemanager.model.enumeration.TokenType;
import com.example.libraryservicemanager.security.JwtConfiguration;
...
import static com.example.libraryservicemanager.model.constant.Constants.*;
...
//                cookie.setSecure(true);
    ...
    public <T> T getTokenData(String token, Function<TokenData, T> tokenFunction) {
        return tokenFunction.apply(
                TokenData.builder()
                        .valid(Objects.equals(userService.getUserById(Long.valueOf(subject.apply(token))).getUserId(), claimsFunction.apply(token).getSubject()))
                        .authorities(authorities.apply(token))
                        .claims(claimsFunction.apply(token))
                        .user(userService.getUserById(Long.valueOf(subject.apply(token))))
                        .build()
        );
    }
